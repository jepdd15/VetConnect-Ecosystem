 ...
 * @param {string} params.clinicTIN     - Clinic TIN for certification block
 * @param {string} params.clinicBAI     - Clinic BAI Registration for certification block
...
function generateMobileVaccinationPassport({ petName, ownerName, clinicName, clinicPhone, clinicAddress, clinicTIN, clinicBAI, petDetails, vaccineRecords, vaccineCatalog }) {
  ...
  const safeTIN = escHtml(clinicTIN || '');
  const safeBAI = escHtml(clinicBAI || '');
...
<body>
  ...
  <div class="cover">
    ...
    ${safeTIN ? `<div class="doc-date" style="margin-top:2px; font-weight:700;">TIN: ${safeTIN}</div>` : ''}
    ${safeBAI ? `<div class="doc-date" style="margin-top:2px; font-weight:700;">BAI REG: ${safeBAI}</div>` : ''}
  ...
  <div class="certification-block">
    ...
    <div class="certification-footer">
      ...
      from ${safeClinic}.${safePhone ? ` Tel: ${safePhone}.` : ''}${safeAddress ? ` Address: ${safeAddress}.` : ''}${safeTIN ? ` TIN: ${safeTIN}.` : ''}${safeBAI ? ` BAI REG: ${safeBAI}.` : ''}
    </div>
  </div>
  ...
  const handleDownloadPassport = async () => {
    try {
      ...
      const html = generateMobileVaccinationPassport({
        ...
        clinicTIN: clinicTIN || '',
        clinicBAI: baiRegistrationNumber || '',
        petDetails: petDoc ? {
