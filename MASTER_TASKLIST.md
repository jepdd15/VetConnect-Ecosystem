...
**Total tasks:** ~801 · **Cancelled/Absorbed:** ~17 · **Active:** ~784
**DONE:** ~635 · **TODO:** ~151 · **Deferred sub-tasks:** 50
...
| T3.129 | EMRDrawer z-index behind ClinicalWorkspace Dialog — The "View Pet EMR History" button (ClinicalWorkspace.jsx ~line 2343, T3.1) calls `setEmrOpen(true)` which opens EMRDrawer.jsx as a right-anchored MUI Drawer. The Drawer renders with default z-index 1200, which is behind the ClinicalWorkspace fullscreen Dialog (z-index 1300). Clicking the button appears to do nothing — the drawer opens but is invisible. Fix: add `sx={{ zIndex: 1400 }}` to the EMRDrawer's root Drawer component (same fix applied to ClinicalAIPanel in T4.110). | P2 | 5 min | — | TODO | Same class of bug as the AI panel z-index fix (T4.110). EMRDrawer is fully implemented and functional — just invisible. |
