...
| T4.155 | Medical history display parity — 7 gaps across admin PatientDashboard and mobile PetHistoryScreen — Comprehensive audit found 7 fields written at CW sign-off but not displayed on one or both surfaces. Fix all: (1) soap.prognosis — written (CW line 1800) but NEVER rendered on either surface. Add "Prognosis: Good/Guarded/Critical/Palliative" line to admin PatientDashboard below the Assessment section (after diagnoses chips + assessmentNotes). Add to mobile PetHistoryScreen inside the discharge summary block as a status pill (same pattern as patientStatus). ~15 min. (2) diagnoses[].notes — per-diagnosis clinical notes written (CW line 1790) but invisible. Admin: add subtitle text below each diagnosis chip showing notes when present. Mobile: include in an expanded diagnosis section. ~20 min. (3) serviceAttribution[] — which vet did which service, written (CW lines 1834-1838) but never shown. Admin: add "Performed by: {staffName}" subtitle per service chip in the record header. Mobile: add vet name per service in the services section. ~15 min. (4) Mobile diagnoses expansion — only first diagnosis shown, "+N more" has no tap handler. Add TouchableOpacity that expands to show all diagnoses with severity inline. ~20 min. (5) Mobile dischargeSummary.recheckIn — missing from Going-Home Instructions. Add "Recheck in: {recheckIn}" line between the follow-up date and the Call Us button. ~10 min. (6) Mobile lab notes + attachment URLs — admin shows lab.notes and lab.attachmentUrl but mobile doesn't. Add notes text below each lab result row. Add attachment link (Linking.openURL) when URL present. ~15 min. (7) Admin nextVisit fallback — top-level nextVisit Timestamp only shown via dischargeSummary. Add standalone "Next Visit: {date}" line when dischargeSummary is absent but nextVisit exists. ~10 min. Files: PatientDashboard.jsx (gaps 1, 2, 3, 7), PetHistoryScreen.js (gaps 1, 2, 3, 4, 5, 6). | P2 | 1.5-2 hrs | T4.141 | TODO | Display completeness — 7 fields written at sign-off but invisible to staff and/or pet owners. Prognosis gap is panelist-visible during normal demo flow. Per-diagnosis notes gap loses clinical context. |
