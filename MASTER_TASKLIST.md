...
**Total tasks:** ~817 · **Cancelled/Absorbed:** ~17 · **Active:** ~800
**DONE:** ~646 · **TODO:** ~156 · **Deferred sub-tasks:** 50
...
| T4.101 | My Bookings: offline support via Firestore persistence + AsyncStorage cache — ClientAppointments.js relies entirely on real-time onSnapshot listeners. If the client opens My Bookings without connectivity, they see a loading spinner forever. Fix: (1) enable Firestore offline persistence via enableMultiTabIndexedDbPersistence (or enablePersistence for RN) in firebaseConfig.js — Firestore automatically caches docs and serves from cache when offline, (2) cache the last-loaded appointments array in AsyncStorage as a fallback for instant display on cold start, (3) show an "Offline — showing cached data" banner when the network is unavailable (use NetInfo from @react-native-community/netinfo, already in Expo). The onSnapshot listener automatically reconnects when connectivity returns. ~3-4 hrs. | P3 | 3-4 hrs | — | TODO | Reliability — clients in areas with spotty PH mobile data see their appointments instantly instead of a blank screen |
