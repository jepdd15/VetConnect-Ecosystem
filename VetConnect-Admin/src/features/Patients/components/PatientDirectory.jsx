  ...
  return (
    <Paper square sx={{ width: 320, display: 'flex', flexDirection: 'column', borderRight: '1px solid #E0E0E0', bgcolor: '#FAFAFA', zIndex: 1, boxShadow: '2px 0 5px rgba(0,0,0,0.02)' }}>
      <Box sx={{ p: 2.5, borderBottom: '1px solid #E0E0E0', bgcolor: 'white' }}>
         <Button variant="contained" startIcon={<PersonAddIcon />} fullWidth sx={{ mb: 2, bgcolor: '#1976D2', '&:hover': {bgcolor: '#1565C0'}, fontWeight:'bold', py: 1.5, boxShadow: '0 4px 12px rgba(25,118,210,0.2)', borderRadius: 2 }} onClick={onNewClient}>
             New Client
         </Button>
         <TextField fullWidth placeholder="Search owner, pet, or phone..." size="small" value={searchText} onChange={onSearchChange} sx={{bgcolor: '#F5F5F5', '& fieldset': {borderColor: '#e0e0e0'}, borderRadius: 1}} InputProps={{ startAdornment: <InputAdornment position="start"><SearchIcon color="disabled"/></InputAdornment>, spellCheck: 'false' }} />
      </Box>
      <List sx={{ overflowY: 'auto', flex: 1, p: 0 }}>
        {owners.map((owner) => (
          <ListItem key={owner.id} disablePadding>
            <ListItemButton selected={selectedId === owner.id} onClick={() => onSelect(owner)} sx={{ borderBottom: '1px solid #F5F5F5', '&.Mui-selected': { bgcolor: '#E3F2FD', borderLeft: '4px solid #1976D2' } }}>
              <ListItemIcon>
                  <Avatar sx={{ bgcolor: selectedId === owner.id ? '#1976D2' : '#E0E0E0', color: selectedId === owner.id ? 'white' : '#777', fontWeight: 'bold' }}>{owner.fullName ? owner.fullName[0] : '?'}</Avatar>
              </ListItemIcon>
              <ListItemText 
                primary={
